...
```{r}
corralation_coefficient = cor(gdp_data$DJIA, gdp_data$GDP)
print(corralation_coefficient)
```
...
```{r}
gdp_data %>% ggplot(aes(x = DJIA, y = GDP)) + geom_point()
labs(title = "DJIA and GDP relationship 2013-2024")
```
...
There is definitely a strong correlation between DJIA and GDP. the correlation between the two variables is 0.9531058, which indicates that there is a strong positive correlation. This means that over the course of 2013-2024 when the Dow Jones Industrial Average or DJIA is rising or falling than the United States' GDP also rises and falls as indicated by the above scatterplot.
